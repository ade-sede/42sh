...
**	allowing us to skip nested expansions, or more simply put, the presence of the `('
**	and `{' characters in the token. Otherwise (`$' and ``' notation), we just
**	check if we stumbled onto the start of another token, or if we found the
**	matching backquote.
**
**	Example:			$( foo (bar))
**							       ^
...
int		match_expand(t_lexer *lex, size_t token_start)
{
	...
	else if (lex->line[token_start] == '$')
	{
		...
		{
			if (IS_INPUT_END(lex->line[lex->index]) || IS_WHITESPACE(lex->line[lex->index]))
				return (1);
		}
	}
	return (0);
