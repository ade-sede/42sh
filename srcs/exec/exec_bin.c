...
void		ft_exec_bin_absolute(t_env *env, const char **argv)
{
	if (access(argv[0], F_OK) == -1)
		exit(investigate_error(NULL, argv[0], "no such file or directory",
					EXIT_FAILURE));
	else if (access(argv[0], X_OK) == -1)
		exit(investigate_error(NULL, argv[0], "permission denied ",
					EXIT_FAILURE));
	else if (execve(argv[0], (char**)argv, env->environ) == -1)
		exit(investigate_error(NULL, argv[0], "command not found",
					EXIT_FAILURE));
	exit(1);
}
...
void		ft_exec_bin_path(t_env *env, const char **argv)
{
	...
	if (access(bin, F_OK) == 0)
	{
		if (access(bin, X_OK) == -1)
			exit(investigate_error(NULL, bin, "permission denied ",
						EXIT_FAILURE));
		else if (execve(bin, (char**)argv, env->environ) == -1)
			exit(investigate_error(NULL, argv[0], "command not found",
						EXIT_FAILURE));
	}
	exit(investigate_error(NULL, (const char *)*argv, "commmand not found",
				EXIT_FAILURE));
}
