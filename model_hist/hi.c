...
//	{
		strftime(result, sizeof(result), "%F ",//format
				localtime(&((t_hist_node *)list->data)->history.timestamp.tv_sec));
		ft_putstr(result);
//	}
	ft_putendl(((t_hist_node *)list->data)->history.line.str);
}
...
*/
int aff_hist(int argc, const char **argv, t_history *h)
{
	int i;
	/*
	if (argc == 1 && h && h->command_list)
		i = h->command_list->node_count;
	else if (argc == 2)
		i = ft_atoi(*(argv + 1));
	else
		return (EXIT_FAILURE);
		*/
	if (h && h->command_list)
		double_lst_for_n(h->command_list->last, h->command_list->node_count, aff_node, -1);
	return (EXIT_SUCCESS);
}
